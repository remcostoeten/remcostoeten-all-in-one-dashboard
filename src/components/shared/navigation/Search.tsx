...
const CommandItem: React.FC<CommandItemProps> = ({ icon, text, shortcut }) => (
    <div className='flex flex-col justify-center px-3.5 py-1 w-full max-md:max-w-full'>
        <div className='hover:cursor-pointer transition-all duration-200 hover:bg-icon-active-background flex gap-0  p-1.5 max-md:flex-wrap max-md:max-w-full rounded-md h-[40px] px-3.5'>
                ...
                <div className='flex flex-col justify-center pr-2 text-xs leading-5 text-white whitespace-nowrap'>
                    <div className='flex gap-0'>
                        ...
                        <div className='flex flex-col justify-center pl-2'>
                            <div className='justify-center py-px rounded border border-solid bg-white bg-opacity-0 border-white border-opacity-10'>
                                {shortcut}
                            </div>
                        </div>
                    </div>
    ...
    useEffect(() => {
        const handleKeyDown = (event: KeyboardEvent) => {
            ...
            if (event.ctrlKey && event.key === 'd') {
                event.preventDefault()
                console.log('Searching with microphone')
            }
            if (event.key === 'Escape') {
                setIsOpen(false)
            }
                    ...
                    >
                        ...
                        >
                            <header className='px-5 pt-4 pb-2.5 text-base border-b border-solid border-white border-opacity-10 text-white text-opacity-60 max-md:max-w-full '>
                                <Input
                                    className='border-none w-full focus:border-none focus:outline-none bg-transparent text-white'
                                    placeholder='Search...'
                                />
                            </header>
                            <section className=' max-h-[650px] overflow-y-auto h-fit'>
                                <CommandItem
                                    icon={<ContactRecordIcon />}
                                    text='Contact record'
                                />
                                <CommandItem
                                    icon={<DepartmentIcon />}
