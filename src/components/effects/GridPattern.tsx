...
const GridBackground: React.FC<GridBackgroundProps> = ({ rayCount, animationDuration }) => {
  ...
  useEffect(() => {
    ...
    const createRayGroup = (position: number, pathCreator: (pos: number) => SVGPathElement, isVertical: boolean) => {
    ...
    if (svgElement) {
      ...
      svgElement.setAttributeNS(ns, 'viewBox', `0 0 ${Math.ceil(width)} ${Math.ceil(height)}`);
      ...
      [...Array(rayCount + 1)].map((_, i) => i * spacing).forEach(pos => {
        svgElement.appendChild(createRayGroup(pos, createVerticalPath, true));
        svgElement.appendChild(createRayGroup(pos, createHorizontalPath, false));
      });
    }
  ...
  return (
    <div className="t">
      <svg ref={svgRef} version="1.1" xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" xmlSpace="preserve" preserveAspectRatio="none" className="grid-rays"></svg>
    </div>
  );
};
