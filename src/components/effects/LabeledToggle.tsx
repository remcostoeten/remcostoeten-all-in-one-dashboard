...
export default function LabeledToggle({
    ...
    return (
        <motion.button
            className={`relative scale-100  h-[25px] w-[45px] cursor-pointer rounded-full outline-offset-2 duration-200 focus-within:outline-red-500 ${
                toggled ? 'bg-sidebar' : 'bg-icon-active-background'
            }`}
                    ...
                    <AnimatePresence>
                        {toggled && (
                            <motion.span
                                ...
                                className='pointer-events-none absolute left-0 top-0 flex h-full items-center px-1.5 text-[8px] font-semibold text-white'
                            >
                                On
                            </motion.span>
                        )}
