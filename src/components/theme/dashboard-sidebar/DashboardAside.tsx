...
const Aside = () => {
    ...
    const filteredDashboardAsideItems = DashboardAsideItems.filter(
        (item) => enabledNavItems[item.name]
    )

    const favouriteItems = filteredDashboardAsideItems.filter(
        (item) => item.isFavourite
    )
    const itemsWithoutFavourites = filteredDashboardAsideItems.filter(
        (item) => !item.isFavourite
    )
