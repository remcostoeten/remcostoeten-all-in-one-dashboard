            ...
            {DashboardAsideItems.map((item) => (
                <div key={item.name} className='flex items-center justify-between'>
                    <div className='flex items-center space-x-2'>
                        <span className='capitalize' aria-label={item.name}>{item.name}</span>
                    </div>
                    <button
                        onClick={() => {
                        ...
                        aria-pressed={enabledNavItems[item.name]}
                    >
                        {enabledNavItems[item.name] ? (
                            <MuiCheckbox checked={true} aria-checked='true' />

                        ) : (
                            <MuiCheckbox checked={false} aria-checked='false' />
                        )}
                    </button>
                </div>
            ))}
