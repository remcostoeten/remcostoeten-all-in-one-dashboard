...
export default function NavSettings() {
    ...
    const popoverContent = (
        <div className="w-56">
            {DashboardAsideItems.map((item, index) => (
                <div
                    key={index}
                    className={`flex items-center px-4 py-3 text-sm ${enabledNavItems[item.name] ? 'text-gray-300' : 'text-gray-500'} hover:bg-gray-700 transition-colors duration-150 ease-in-out cursor-pointer`}
