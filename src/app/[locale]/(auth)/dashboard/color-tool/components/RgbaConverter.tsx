...
export default function RgbaConverter({
    ...
    const handlePaste = (e: React.ClipboardEvent<HTMLInputElement>, channel: 'r' | 'g' | 'b' | 'a') => {
        e.preventDefault()
        ...
        if (values.length >= 3 && values.every(v => !isNaN(v))) {
            if (channel === 'r' || channel === 'g' || channel === 'b') {
                ...
                onChange({ ...value, [channel]: Math.min(255, Math.max(0, values[index])) })
            ...
            const newValue = channel === 'a' ? Math.min(1, Math.max(0, Number(pastedText))) : Math.min(255, Math.max(0, Number(pastedText)))
            onChange({ ...value, [channel]: newValue })
        }
    }

    const handleInputChange = (channel: 'r' | 'g' | 'b' | 'a', newValue: number) => {
        onChange({ ...value, [channel]: newValue })
    }
        ...
        <Flex direction='col' items='center' className="space-y-2">
            ...
            {['r', 'g', 'b', ...(isRgba ? ['a'] : [])].map((channel) => (
                <Flex key={channel} direction='col' items='center'>
                    <label className='text-text py-0.5'>{channel.toUpperCase()}</label>
                    <Input
                        className={`${sizes[size]} items-center text-center`}
                        type='number'
                        value={value && value[channel as keyof typeof value] !== undefined ? value[channel as keyof typeof value] : 0}
                        onChange={(e) => handleInputChange(channel as 'r' | 'g' | 'b' | 'a', Number(e.target.value))}
                        onPaste={(e) => handlePaste(e, channel as 'r' | 'g' | 'b' | 'a')}
                        placeholder={size === 'large' ? '#fff' : channel.toUpperCase()}
                        min={0}
                        max={channel === 'a' ? 1 : 255}
